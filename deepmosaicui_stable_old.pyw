...
class DeepMosaicsUI:
    ...
    def create_widgets(self):
        ...
        # Min Mosaic Area
        min_area_label = ctk.CTkLabel(scroll_frame, text="Min Detection Area:")
        min_area_label.grid(row=current_row, column=0, pady=5, sticky="w")
        self.min_area_entry = ctk.CTkEntry(scroll_frame, placeholder_text="300")
        self.min_area_entry.grid(row=current_row, column=1, pady=5, sticky="ew", padx=(10, 0))
        self.min_area_entry.insert(0, "300")
        min_area_note = ctk.CTkLabel(scroll_frame,
            text="Pixel area threshold for mask components. Lower = detect smaller regions.",
            font=ctk.CTkFont(size=10), text_color=("gray50", "gray60"), justify="left")
        min_area_note.grid(row=current_row + 1, column=0, columnspan=2,
                           pady=(0, 4), sticky="w", padx=(4, 0))
        current_row += 2

        # Min Mosaic Size
        min_size_label = ctk.CTkLabel(scroll_frame, text="Min Clean Size:")
        min_size_label.grid(row=current_row, column=0, pady=5, sticky="w")
        self.min_size_entry = ctk.CTkEntry(scroll_frame, placeholder_text="100")
        self.min_size_entry.grid(row=current_row, column=1, pady=5, sticky="ew", padx=(10, 0))
        self.min_size_entry.insert(0, "100")
        min_size_note = ctk.CTkLabel(scroll_frame,
            text="Min bounding-box half-size (px) to attempt cleaning. Lower = process smaller areas.",
            font=ctk.CTkFont(size=10), text_color=("gray50", "gray60"), justify="left")
        min_size_note.grid(row=current_row + 1, column=0, columnspan=2,
                           pady=(0, 4), sticky="w", padx=(4, 0))
        current_row += 2
        ...
        # ── Separator ────────────────────────────────────────────────
        ctk.CTkLabel(scroll_frame, text="", height=10).grid(
            row=current_row, column=0, columnspan=2)
        current_row += 1

        # ── Experimental Section ──────────────────────────────────────
        exp_section = ctk.CTkLabel(scroll_frame, text="⚗ Experimental — Post-Processing",
                                   font=ctk.CTkFont(size=18, weight="bold"))
        exp_section.grid(row=current_row, column=0, columnspan=2, pady=(10, 4), sticky="w")
        current_row += 1

        exp_note = ctk.CTkLabel(
            scroll_frame,
            text="These filters run on the cleaned patch before it is composited back.\n"
                 "Stack them or use individually — freq inject → bilateral → luma USM.",
            font=ctk.CTkFont(size=11),
            text_color=("gray50", "gray60"),
            justify="left")
        exp_note.grid(row=current_row, column=0, columnspan=2, pady=(0, 10), sticky="w")
        current_row += 1

        # Helper: make a labelled slider row with enable checkbox
        def _sharpen_row(label_text, description, cb_var, amount_var,
                         from_, to_, steps, default_amount):
            nonlocal current_row
            cb = ctk.CTkCheckBox(scroll_frame, text=label_text, variable=cb_var)
            cb.grid(row=current_row, column=0, columnspan=2, pady=(6, 0), sticky="w")
            current_row += 1

            desc_lbl = ctk.CTkLabel(scroll_frame, text=description,
                                    font=ctk.CTkFont(size=10),
                                    text_color=("gray50", "gray60"), justify="left")
            desc_lbl.grid(row=current_row, column=0, columnspan=2, pady=(0, 4), sticky="w",
                          padx=(24, 0))
            current_row += 1

            amt_lbl = ctk.CTkLabel(scroll_frame, text="Amount:")
            amt_lbl.grid(row=current_row, column=0, pady=2, sticky="w", padx=(24, 0))

            slider_frame = ctk.CTkFrame(scroll_frame, fg_color="transparent")
            slider_frame.grid(row=current_row, column=1, pady=2, sticky="ew", padx=(10, 0))
            slider_frame.grid_columnconfigure(0, weight=1)

            slider = ctk.CTkSlider(slider_frame, from_=from_, to=to_,
                                   variable=amount_var, number_of_steps=steps)
            slider.grid(row=0, column=0, sticky="ew")

            val_lbl = ctk.CTkLabel(slider_frame, text=f"{default_amount:.2f}", width=40)
            val_lbl.grid(row=0, column=1, padx=(6, 0))
            amount_var.trace_add("write",
                lambda *_, v=amount_var, l=val_lbl: l.configure(text=f"{v.get():.2f}"))
            current_row += 1

            # Grey out when unchecked
            widgets = [amt_lbl, slider, val_lbl]
            def _toggle(cb_v=cb_var, ws=widgets):
                s = "normal" if cb_v.get() else "disabled"
                c = ("gray10", "gray90") if cb_v.get() else ("gray60", "gray40")
                for w in ws:
                    try:
                        w.configure(state=s, text_color=c)
                    except Exception:
                        try:
                            w.configure(state=s)
                        except Exception:
                            pass
            cb_var.trace_add("write", lambda *_, t=_toggle: t())
            _toggle()  # initial state
            return cb, slider

        # 1. Frequency Injection
        self.freq_inject_var    = ctk.BooleanVar(value=False)
        self.freq_amount_var    = ctk.DoubleVar(value=0.30)
        _sharpen_row(
            "Frequency Injection",
            "Borrows sharp edges from the original mosaic — most impactful,\n"
            "start here. (0.1 subtle → 0.6 strong; above ~0.6 grid lines may show)",
            self.freq_inject_var, self.freq_amount_var,
            0.05, 0.80, 15, 0.30)

        # 2. Bilateral Sharpening
        self.bilateral_sharpen_var    = ctk.BooleanVar(value=False)
        self.bilateral_amount_var     = ctk.DoubleVar(value=0.50)
        _sharpen_row(
            "Bilateral Sharpening",
            "Edge-preserving sharpening — less haloing than luma USM,\n"
            "good complement to freq injection.",
            self.bilateral_sharpen_var, self.bilateral_amount_var,
            0.10, 2.00, 19, 0.50)

        # 3. Luma Sharpening (USM)
        self.luma_sharpen_var    = ctk.BooleanVar(value=False)
        self.luma_amount_var     = ctk.DoubleVar(value=1.00)
        _sharpen_row(
            "Luma Sharpening (Unsharp Mask)",
            "Classic unsharp mask on the Y channel only — crisp final pass,\n"
            "apply last. (1.0 normal → 2.0 aggressive)",
            self.luma_sharpen_var, self.luma_amount_var,
            0.10, 3.00, 29, 1.00)
    ...
    def generate_command(self):
        ...
        min_area = self.min_area_entry.get().strip()
        if min_area and min_area != "300":
            cmd.extend(["--min_mosaic_area", min_area])

        min_size = self.min_size_entry.get().strip()
        if min_size and min_size != "100":
            cmd.extend(["--min_mosaic_size", min_size])
        ...
        # Experimental sharpening — always emit amount so the slider value is honoured
        if self.freq_inject_var.get():
            cmd.append("--freq_inject")
            cmd.extend(["--freq_inject_amount", str(round(self.freq_amount_var.get(), 2))])

        if self.bilateral_sharpen_var.get():
            cmd.append("--bilateral_sharpen")
            cmd.extend(["--bilateral_sharpen_amount", str(round(self.bilateral_amount_var.get(), 2))])

        if self.luma_sharpen_var.get():
            cmd.append("--luma_sharpen")
            cmd.extend(["--luma_sharpen_amount", str(round(self.luma_amount_var.get(), 2))])
    ...
    def clear_all(self):
        ...
        # Experimental
        self.freq_inject_var.set(False)
        self.freq_amount_var.set(0.30)
        self.bilateral_sharpen_var.set(False)
        self.bilateral_amount_var.set(0.50)
        self.luma_sharpen_var.set(False)
        self.luma_amount_var.set(1.00)
